...
def init_db():
    ...
    # Nuevas tablas para compartir playlists y comentarios
    conn.execute('''
...
# Función para comprobar si una playlist es pública
def is_playlist_public(playlist_id):
    conn = get_db_connection()
    shared = conn.execute(
        'SELECT is_public FROM shared_playlists WHERE playlist_id = ?',
        (playlist_id,)
    ).fetchone()
    conn.close()
    
    if shared and shared['is_public'] == 1:
        return True
    return False

# Función para comprobar si una playlist pertenece a un usuario
def is_playlist_owner(playlist_id, user_id):
    conn = get_db_connection()
